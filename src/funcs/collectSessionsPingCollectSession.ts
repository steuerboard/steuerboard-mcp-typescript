...
import {
  ...
  PingCollectSessionRequest$zodSchema,
  PingCollectSessionResponse,
  PingCollectSessionResponse$zodSchema,
} from "../models/pingcollectsessionop.js";
 ...
 */
export function collectSessionsPingCollectSession(
  client$: SteuerboardCore,
  request: PingCollectSessionRequest,
...
> {
  const parsed$ = safeParse(
    request,
    (value$) => PingCollectSessionRequest$zodSchema.parse(value$),
  ...
  const path$ = pathToFunc("/collect/sessions/{token}/ping")(
    pathParams$,
  );
  ...
  const context = {
    ...
    operationID: "pingCollectSession",
    oAuth2Scopes: [],
  ...
  const [result$] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(202, PingCollectSessionResponse$zodSchema, {
      key: "PingSessionResponse",
    }),
